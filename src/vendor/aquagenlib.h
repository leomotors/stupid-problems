 ...
 */
template <typename T> vector<T> pick_set(T n, int k) {
    ensure(k >= 0 && k <= n);
 ...
 * The output is not sorted in any ways.
 */
template <typename T> vector<T> pick_set(T min_n, T max_n, int k) {
...
vector<int> gen_parent_tree(int n, int min_k, int max_k, int r = -1) {
    ...
    for (int i = 0; i < n; ++i)
        new_parents[new_label[i]] =
            parents[i] == -1 ? -1 : new_label[parents[i]];
