    ...
    while (!hx.empty()) {
        ...
        for (auto x : node[o.v]) {
            ...
            if (x.st == s)
                continue;
            bool chx = true;
            auto it1 = disc[x.st].upper_bound(new_high);
            --it1;
            if (*it1 >= new_low && mem_low[x.st][*it1] >= new_low)
                continue;
            disc[x.st].insert(new_high);
            mem_low[x.st][new_high] = new_low;
